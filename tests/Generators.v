...
(** We define a propositional predicate [occ i j] which denotes
   ...
   *)
Inductive occ : nat -> nat -> Prop :=
| mk_occ : forall i j, occ i j.
...
Definition hole (n : nat) : Prop :=
  match n with
    | O => False
    | S n0 => 
      everyone_home n0 /\ everyone_happy n0
  end.
...
(** We define a family a boolean variables indexed by an integer [x i]. *)
