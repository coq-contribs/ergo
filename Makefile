...
OCAMLLIBS?=-I "$(COQLIB)/user-contrib/Counting"\
...
VOFILES2=$(patsubst tests/%,%,$(filter tests/%,$(VOFILES)))
...
NATIVEFILES2=$(patsubst tests/%,%,$(filter tests/%,$(NATIVEFILES)))
...
install:$(if $(HASNATDYNLINK_OR_EMPTY),install-natdynlink)
	cd "tests" && for i in $(NATIVEFILES2) $(GLOBFILES2) $(VFILES2) $(VOFILES2); do \
...
uninstall_me.sh: Makefile
	...
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Tests && rm -f $(NATIVEFILES2) $(GLOBFILES2) $(VFILES2) $(VOFILES2) && \\\nfor i in $(CMAFILESINC) $(CMIFILESINC) $(CMOFILESINC); do rm -f "`basename "$$i"`"; done && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Tests" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Ergo && rm -f $(NATIVEFILES1) $(GLOBFILES1) $(VFILES1) $(VOFILES1) && \\\nfor i in $(CMAFILESINC) $(CMIFILESINC) $(CMOFILESINC); do rm -f "`basename "$$i"`"; done && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Ergo" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
