...
OCAMLLIBS?=-I "src"\
  -I "$(COQLIB)/user-contrib/Containers"\
  -I "$(COQLIB)/user-contrib/Nfix"\
  -I "$(COQLIB)/user-contrib/Counting"
COQLIBS?=\
  -R "tests" Tests\
  -R "theories" Ergo\
  -I "src"\
...
VO=vo
VOFILES:=$(VFILES:.v=.$(VO))
VOFILES1=$(patsubst tests/%,%,$(filter tests/%,$(VOFILES)))
...
NATIVEFILES1=$(patsubst tests/%,%,$(filter tests/%,$(NATIVEFILES)))
NATIVEFILES2=$(patsubst theories/%,%,$(filter theories/%,$(NATIVEFILES)))
...
install-natdynlink:
	...
	for i in $(CMXSFILESINC); do \
	 install -m 0755 $$i "$(DSTROOT)"$(COQLIBINSTALL)/Ergo/`basename $$i`; \
	done
	install -d "$(DSTROOT)"$(COQLIBINSTALL)/Tests; \
	for i in $(CMXSFILESINC); do \
	 install -m 0755 $$i "$(DSTROOT)"$(COQLIBINSTALL)/Tests/`basename $$i`; \
	done
...
install:$(if $(HASNATDYNLINK_OR_EMPTY),install-natdynlink)
	cd "theories" && for i in $(NATIVEFILES2) $(GLOBFILES2) $(VFILES2) $(VOFILES2); do \
	...
	cd "tests" && for i in $(NATIVEFILES1) $(GLOBFILES1) $(VFILES1) $(VOFILES1); do \
...
uninstall_me.sh: Makefile
	...
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Ergo && rm -f $(NATIVEFILES2) $(GLOBFILES2) $(VFILES2) $(VOFILES2) && \\\nfor i in $(CMAFILESINC) $(CMIFILESINC) $(CMOFILESINC); do rm -f "`basename "$$i"`"; done && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Ergo" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Tests && rm -f $(NATIVEFILES1) $(GLOBFILES1) $(VFILES1) $(VOFILES1) && \\\nfor i in $(CMAFILESINC) $(CMIFILESINC) $(CMOFILESINC); do rm -f "`basename "$$i"`"; done && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Tests" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
